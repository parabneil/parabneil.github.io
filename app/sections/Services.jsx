...
const Services = () => {
  ...
  return (
    ...
    >
      ...
      >
        {services?.map(({ id, icon, title, description, services }, index) => (
          <motion.div
            key={id || index}
          ...
          >
            <div className="flex items-center justify-center">
              {/* Icon Container */}
              <div className="w-12 h-12 flex items-center justify-center rounded-2xl bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-400 mr-6 group-hover:bg-cyan-700 group-hover:text-white transition-colors duration-300">
                {/* Check if icon is a component or a string path */}
                {icon}
              </div>

              <div>
                <h3 className="text-lg font-bold text-gray-800 dark:text-white group-hover:text-cyan-700 dark:group-hover:text-cyan-400 transition-colors">
                  {title}
                </h3>
